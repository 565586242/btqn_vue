<template>
  <div class="body">
    ...
    <input-list v-model="formData.recode" type="number" placeholder="请输入邀请码" :iconUrl="inviteIcon"></input-list>
    <block-button @click.native="registerClick">立即注册</block-button>
    <p>我已阅读并同意《抱团取暖服务协议》</p>
  </div>
</template>
...
import { homeregister,getphonecode } from '@/ajax/api';
export default {
  data() {
    return {
      ...
      inviteIcon: require('assets/image/login_tj@2x.png'),
      time: 0,
      timeInterval: null,
      formData: {
        name: '',
  ...
  methods: {
    registerClick() {
      const _this = this
      homeregister(_this.formData).then(res => {
        if(res && res.code == 200){
          Toast({
            message: res.msg,
            duration: 1000
